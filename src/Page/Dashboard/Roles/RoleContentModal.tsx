...
const RoleContentModal: React.FC<RoleContentModalProps> = ({ resetData, setResetData, setOpenDiaLog }) => {
  ...
  const handleGetValueSelected = (value: any) => {
    setDataSubmit((prevData: any) => ({ // Sử dụng hàm setState với callback function để tránh lỗi trong trường hợp truy cập trực tiếp vào state
      ...prevData,
  ...
  return (
    <>
      ...
      <fieldset className="mb-[15px] flex flex-col gap-5">
        ...
        <Input
          onChange={(event: any) => handelChangeInput('roleName', event.target.value)}
          id="name"
        />
      </fieldset>
      <div>
        {
          isLoading ?
            ...
            :
            <DataTableRoute handleGetValueSelected={(value: any) => handleGetValueSelected(value)} columns={columns} data={routes} />
        }
      </div>
